...
--
-- CONDITIONNEL (IF EXISTS) : la migration ne doit pas présupposer l'existence du
-- rôle. En prod l'ordre est provision→migrate (rôle présent), mais la suite
-- d'isolation applique migrate→provision (rôle absent à ce stade) ; un REVOKE nu
-- ferait alors échouer la migration. Le bloc DO ne révoque que si le rôle existe.
-- ⚠️ LIMITE CONNUE (dette, voir TODOS) : tygr_app.sql accorde DELETE ON ALL
-- TABLES. Selon l'ordre, ce REVOKE peut être ré-écrasé par ce GRANT global
-- (tests : migrate→provision ⇒ le GRANT passe APRÈS et ré-accorde DELETE). La
-- garantie tombstone DÉFINITIVE exige de retirer DELETE sur ces 2 tables au
-- niveau du provisioning, pas de l'accorder puis le révoquer — chantier dédié
-- (touche la surface sécurité du provisioning). Ce REVOKE conditionnel ne casse
-- aucun ordre et pose l'intention ; il ne suffit pas seul.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tygr_app') THEN
    REVOKE DELETE ON "transactions_cache" FROM "tygr_app";
    REVOKE DELETE ON "transactions_cache_2024" FROM "tygr_app";
    REVOKE DELETE ON "transactions_cache_2025" FROM "tygr_app";
    REVOKE DELETE ON "transactions_cache_2026" FROM "tygr_app";
    REVOKE DELETE ON "transactions_cache_2027" FROM "tygr_app";
    REVOKE DELETE ON "transactions_cache_default" FROM "tygr_app";
