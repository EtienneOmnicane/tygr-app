...
-- quatre triggers (UPDATE/DELETE + TRUNCATE, par table). Le code N-1 ignore ces
-- tables. Aucune fenêtre expand/contract.
--
...
--   (3) triggers ci-dessous : la seule défense indépendante du privilège ET du
--       chemin (cascade FK, DELETE direct, TRUNCATE, même sous l'owner) —
--       (3a) BEFORE UPDATE OR DELETE ... FOR EACH ROW
--       (3b) BEFORE TRUNCATE ... FOR EACH STATEMENT (trigger DISTINCT, cf. infra)
--
...
-- GARDE (3a) — APPEND-ONLY STRICT par TRIGGER, sur UPDATE et DELETE.
...
-- C'est une défense indépendante du privilège ET du chemin : elle mord sur
...
-- GARDE (3b) — APPEND-ONLY STRICT par TRIGGER, sur TRUNCATE.
--
-- POURQUOI UN SECOND TRIGGER. Un trigger `BEFORE UPDATE OR DELETE ... FOR EACH ROW`
-- n'est JAMAIS déclenché par `TRUNCATE` : PostgreSQL traite TRUNCATE comme une
-- opération de niveau TABLE, pas de niveau ligne, et exige un trigger distinct
-- `BEFORE TRUNCATE ... FOR EACH STATEMENT` (un trigger TRUNCATE `FOR EACH ROW` est
-- d'ailleurs refusé à la création). Sans (3b), `TRUNCATE audit_events` sous l'owner
-- viderait l'audit réglementaire SANS lever — vérifié empiriquement.
--
-- Le privilège seul ne suffit pas non plus : le rôle applicatif n'a certes pas
-- TRUNCATE (jamais accordé), mais une migration de réparation, un script d'admin ou
-- un `psql` sous l'owner passeraient. C'est exactement la raison d'être de la garde
-- par trigger : être indépendante du privilège ET du chemin (règle 8 — « aucun
-- UPDATE/DELETE, MÊME EN MIGRATION DE RÉPARATION : on écrit un événement correctif »).
--
-- ⚠️ LE CHEMIN VRAIMENT DANGEREUX N'EST PAS `TRUNCATE consent_records`. C'est
-- `TRUNCATE workspaces CASCADE` : la cascade se propage aux tables qui référencent
-- la cible par FK, donc à consent_records (workspace_id → workspaces). Vérifié en
-- retirant le trigger : le TRUNCATE passe et vide l'audit **sans lever**. Personne
-- n'aurait écrit « TRUNCATE consent_records » dans une migration ; « TRUNCATE
-- workspaces CASCADE » pour réinitialiser un env de test, en revanche, oui. C'est
-- le scénario #3bis (cascade FK contournant le privilège) transposé à TRUNCATE.
-- Preuve : test 4g de tests/isolation/audit-append-only-isolation.test.ts.
--
-- Fonction DÉDIÉE, et non réutilisation de tygr_refuser_mutation_append_only() : le
-- message de cette dernière annonce « ni UPDATE ni DELETE » et mentirait sur la cause.
-- Un message d'erreur exact est ce qui permet au prochain développeur de comprendre
-- qu'il doit écrire un événement correctif, pas désarmer la garde.
--
-- Elle est GÉNÉRIQUE (nommée par TG_TABLE_NAME) : les autres tables append-only du
-- projet (transactions_cache + partitions, balance_history, categorization_audit)
-- l'adopteront sans la redéfinir, en migration 0022.
--
-- ⚠️ CES TROIS TABLES SONT EXPOSÉES, ET CE N'EST PAS RÉGLÉ ICI. Mesuré : `TRUNCATE
-- balance_history` et `TRUNCATE categorization_audit` PASSENT sous l'owner (0 ligne,
-- aucune erreur) ; `TRUNCATE bank_accounts CASCADE` emporte transactions_cache ET
-- balance_history. Correction = migration 0022 dédiée (branche séparée) : ce lot est
-- le socle d'Epic 1 dont L3.2 dépend, on n'y greffe pas la protection de tables sans
-- rapport. NE PAS conclure de l'écran de fumée ci-dessous que le problème est réglé :
-- depuis cette migration, `TRUNCATE workspaces CASCADE` échoue — mais seulement parce
-- que la cascade traverse consent_records et que SON trigger annule la transaction.
-- Les trois autres tables ne sont pas protégées, elles sont MASQUÉES.
CREATE OR REPLACE FUNCTION tygr_refuser_truncate_append_only()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION
    'append_only_no_truncate: la table % est append-only — TRUNCATE interdit (écrire un événement correctif)',
    TG_TABLE_NAME
    USING ERRCODE = 'check_violation';
END;
$$;--> statement-breakpoint
DROP TRIGGER IF EXISTS "consent_records_no_truncate" ON "consent_records";--> statement-breakpoint
CREATE TRIGGER "consent_records_no_truncate" BEFORE TRUNCATE ON "consent_records"
	FOR EACH STATEMENT EXECUTE FUNCTION tygr_refuser_truncate_append_only();--> statement-breakpoint
DROP TRIGGER IF EXISTS "audit_events_no_truncate" ON "audit_events";--> statement-breakpoint
CREATE TRIGGER "audit_events_no_truncate" BEFORE TRUNCATE ON "audit_events"
	FOR EACH STATEMENT EXECUTE FUNCTION tygr_refuser_truncate_append_only();
