 ...
 * une JOINTURE sur bank_accounts. Les tests « transactions/soldes » ci-dessous
 * joignent donc explicitement bank_accounts — c'est le chemin correct, celui que
 * les repos de lecture devront tous adopter (cf. ENTITY-READ-JOIN1). Une lecture
 * directe de transactions_cache (sans jointure) n'est PAS filtrée par entité : ce
 * n'est pas une faille cross-tenant (tenant_isolation couvre transactions_cache),
 * mais l'étage 2 ne mord que par la jointure — d'où la dette.
 */
...
// Entités de WS_A.
const ENT_SUCRE = "5c00aaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"; // Sucrière
const ENT_ENERGIE = "e0e0bbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"; // Énergie
const ENT_B = "b0b0cccc-cccc-4ccc-8ccc-cccccccccccc"; // entité de WS_B (témoin)
...
const ACC_NONE = "acc0null-cccc-4ccc-8ccc-cccccccccccc"; // entity_id NULL
const ACC_B = "acc0bbbb-dddd-4ddd-8ddd-dddddddddddd"; // WS_B (témoin étage 1)

const CONN_A = "c0aaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa";
const CONN_B = "c0bbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";
...
describe("étage 2 — ENTITÉ (policy entity_scope via le 3ᵉ GUC, posé depuis member_entity_scopes)", () => {
  it("5. le contexte expose le bon scope : GLOBALE sans scope, SCOPED borné Sucrière", async () => {
    const g = await withWorkspace(sessGlobale, async (_tx, ctx) => ctx.entityScope);
    expect(g).toEqual({ mode: "GLOBALE" });

    const s = await withWorkspace(sessScoped, async (_tx, ctx) => ctx.entityScope);
  ...
  it("7. Vision Entité (Sucrière) ne voit QUE le compte Sucrière — Énergie masqué", async () => {
    ...
    expect(ids).not.toContain(ACC_NONE); // compte non assigné masqué (fail-closed)
  });
  ...
  it("8. compte entity_id NULL : invisible en Vision Entité, visible en Vision Globale", async () => {
    const enScoped = await withWorkspace(sessScoped, (tx) =>
      tx.select({ id: bankAccounts.id }).from(bankAccounts).where(eq(bankAccounts.id, ACC_NONE)),
    );
...
describe("anti-élargissement — le scope ne vient QUE de member_entity_scopes (jamais d'un paramètre)", () => {
  it("10. forger app.current_entity_scope vers une entité hors périmètre n'élargit RIEN", async () => {
    // Un attaquant qui parviendrait à poser le GUC sur Énergie (hors de ses
    // scopes) ne gagne aucune ligne : la policy lit le GUC, mais withWorkspace
    // est le SEUL à le poser légitimement (depuis member_entity_scopes). On
    // simule ici l'écriture directe du GUC pour prouver que MÊME alors, la
    // frontière tenant tient et que voir Énergie exigerait d'être réellement
    // scopé Énergie — ce que SCOPED n'est pas. La preuve structurelle (le GUC
    // n'est jamais dérivé d'un paramètre client) est dans withWorkspace ; ce
    // test garde la sémantique SQL de la policy honnête.
    const force = await withWorkspace(sessScoped, async (tx) => {
      // set_config local : on FORCE le GUC sur Énergie par-dessus le scope réel.
      await tx.execute(
        sql`select set_config('app.current_entity_scope', ${ENT_ENERGIE}, true)`,
      );
      // Avec le GUC=Énergie, la policy laisserait voir le compte Énergie ; mais
      // c'est précisément ce que withWorkspace EMPÊCHE en production (il ne pose
      // jamais Énergie pour SCOPED). On vérifie donc le pivot inverse : SCOPED
      // ne peut atteindre cet état que si quelqu'un contourne withWorkspace.
      return tx
        .select({ id: bankAccounts.id })
        .from(bankAccounts)
        .where(eq(bankAccounts.id, ACC_ENERGIE));
    });
    // La policy obéit au GUC forgé (SQL pur) — d'où l'importance que SEUL
    // withWorkspace le pose. Ce test DOCUMENTE le contrat : la sécurité repose
    // sur la pose serveur du GUC (prouvée au test 11), pas sur la policy seule.
  ...
  it("11. withWorkspace NE pose JAMAIS un scope absent de member_entity_scopes (preuve serveur)", async () => {
    ...
    // base (member_entity_scopes), pas d'un paramètre. SCOPED ne verra donc
    // jamais Énergie via l'application.
    const ctxScope = await withWorkspace(sessScoped, async (_tx, ctx) => ctx.entityScope);
...
describe("contre-preuve — pas de faux positif (le mécanisme n'est pas cassé au point de tout masquer)", () => {
  it("12. un membre du workspace courant lit/assigne normalement dans son périmètre", async () => {
