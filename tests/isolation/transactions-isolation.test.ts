...
describe("filtres", () => {
  ...
  it("compose recherche + compte + statut (filtres cumulés en AND)", async () => {
    // "ACME" matche T4 (Salaire ACME) et T6 (… ACME) ; restreint au statut
    // NON_CATEGORISE (T4, T6 le sont) et au compte ACC_A → les deux ressortent, mais
    // ce test PROUVE que les prédicats se cumulent (un statut COMPLET donnerait vide).
