...
### 11. Sobriété du code (YAGNI)
- **Le minimum de code qui résout le problème posé, rien de spéculatif.** Aucune
  fonctionnalité au-delà de la demande ; aucune abstraction pour un usage unique ;
  aucune « flexibilité » / configurabilité non demandée ; aucune gestion d'erreur pour
  un scénario impossible. Si 200 lignes se réduisent à 50, on réécrit.
- Test : « un Staff Engineer dirait-il que c'est sur-conçu ? » Si oui → simplifier.
  Ne PAS confondre avec la rigueur sécurité/isolation (règles 2, 3, 8) : celle-là
  n'est jamais « du superflu » et ne se coupe jamais au nom de la sobriété.
- L'alternative plus simple se propose AVANT de coder (règle 10), pas après ; une fois
  la voie tranchée, on ne re-complexifie pas en cours de route (règle 7, anti-scope-creep).

### 12. Changements chirurgicaux (hygiène de diff)
- **Ne toucher QUE ce qui sert la demande. Chaque ligne changée trace vers le ticket.**
  Interdit d'« améliorer » du code adjacent, des commentaires ou du formatage non liés ;
  interdit de refactorer ce qui n'est pas cassé ; on épouse le style existant même si on
  ferait autrement.
- On nettoie SON propre désordre : retirer les imports/variables/fonctions que NOS
  changements rendent orphelins. Le dead code PRÉEXISTANT se **signale** (entrée TODOS.md
  si dette, règle 9), il ne se supprime PAS sans demande.
- Corollaire commit : `git add` sélectif, jamais `git add -A` (déjà règle 7) — le diff
  d'un PR ne contient que le geste demandé.
