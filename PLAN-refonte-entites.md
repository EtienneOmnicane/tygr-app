...
>
> **Phase : IMPLÉMENTATION — TERMINÉE (2026-07-13).** Les 6 lots (L0→L5) sont livrés sur
> `feature/refonte-entites-ia`, ainsi que l'arbitrage §12. Suite complète verte
> (1057 tests / 71 fichiers) ; Visual QA (Gate 4) sur 5 routes de démo ; 4 cross-reviews
> indépendantes passées et leurs constats corrigés (§11, §11-bis). L'agent s'ARRÊTE à la
> PR poussée — Etienne ouvre et merge (Human-in-the-Loop).
>
...
## 12. ✅ Résidu — TRANCHÉ (Etienne, 2026-07-13) : un ADMIN n'est jamais scopé
...
### ✅ DÉCISION (Etienne, 2026-07-13) — la CAUSE est fermée

> *« Interdis de scoper un ADMIN dans definirScopesMembre — garde de rôle en plus de la
> membership — avec les 2 cas d'isolation. Tes gardes d'écriture et l'avertissement
> d'affichage restent en fail-safe. Rôle et scope restent orthogonaux : on refuse juste la
> combinaison ADMIN+scopé, aucun cas d'usage au MVP. »*

Livré : `AdminNonScopableError`, portée par le rôle de la **CIBLE** (jamais `ctx.role`).
Posée sur les **deux** axes — `definirScopesMembre` (entité) **et** `definirScopesFinsMembre`
(compte/party) : fermer un seul axe créerait un cul-de-sac (un ADMIN scopé finement garderait
des lectures partielles et verrait ses écritures refusées, sans recours). `octroyerScopeFin`
et `creerMembreAvecScopes` **délèguent** → garde héritée, un seul point de vérité par axe.
**Retirer** un périmètre reste permis : c'est le chemin de réparation d'un état hérité.
8 cas d'isolation (46/47/48 · 17/18/19 · 7/8). Dette de vigilance tracée :
`ROLE-PROMOTION-SCOPE1` (aucun chemin de promotion de rôle n'existe aujourd'hui).

Les fail-safes **restent** (défense en profondeur) : la garde applicative empêche de CRÉER
l'état, elle n'efface pas ceux déjà en base.

---

**Ce que le chantier faisait AVANT cet arbitrage** (pour mémoire) :
